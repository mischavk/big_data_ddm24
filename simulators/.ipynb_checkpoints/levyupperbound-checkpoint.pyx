from libc.math cimport log, log1p, M_PI, tan, sqrt, sin, pow, cos, fabs
...
cdef api double levy_trial(double v, double sv, double zr, double szr, double a, 
    ...
    ndtplus_old = ndtplus	
    ndtplus = ndtplus - 0.5*sndt + sndt * random_uniform() # draw ndt from block
    if (ndtplus_old == ndtminus):
        ndtminus = ndtplus
    ...
    cdef double random_no = 1
    cdef double half_rand = RAND_MAX/2
    cdef double noise = 0
    # Simulate a single DM path
    while (x > 0 and x < a and n_steps < max_steps):
        random_no = rand() 
        # DDM equation
        if (random_no > half_rand):       
          noise = rhs*random_levy(c, alpha)
          if (noise < 0):
            noise = noise * -1
        else:       
          rhs = pow(dt, 1. / 2)          
          noise = rhs*random_levy(c, 2)
          if (noise > 0):
            noise = noise * - 1
        # Increment step
        x = x + vdt + noise
